...
export default function Employees() {
  const { team, models, setPct, addMember, profile } = useStore();
  ...
  return (
    <div className="p-6">
      ...
      <div className="card p-[18px]">
        ...
        {team.length === 0 ? <div className="text-muted2 text-[13px] py-6 text-center">Aucun membre pour l'instant.</div> : (
          <table className="w-full">
            <thead><tr className="text-left text-[11px] uppercase tracking-wide text-muted2">{["Membre", "Rôle", "CA généré", "Commission %", "Sa part"].map((h, i) => <th key={h} className={`py-2.5 px-3.5 border-b border-line font-semibold ${i === 4 ? "text-right" : ""}`}>{h}</th>)}</tr></thead>
            <tbody>
              {team.map((e) => (
        ...
        <div className="text-[12px] text-muted mt-3">💡 La part se recalcule automatiquement (CA généré × %). Ex : 10 000 € × 8% = 800 €.</div>
      </div>
