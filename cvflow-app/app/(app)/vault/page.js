...
export default function Vault() {
  const { models, vault, activeModel, setActiveModel, addMedia, addFolder } = useStore();
  ...
  return (
    <div className="p-6">
      <div className="flex items-center mb-1.5">
        ...
        <div className="badge text-muted bg-panel border border-line px-3 py-1.5">Modèle : <select value={current} onChange={(e) => setActiveModel(e.target.value)} className="bg-transparent text-txt font-semibold ml-1 outline-none">{models.map((x) => <option key={x.id} value={x.id}>{x.name}</option>)}</select></div>
        <button onClick={() => { setFolderName(""); setFolderOpen(true); }} className="btn-ghost ml-2.5 px-3 py-2 text-[12px] font-semibold">＋ Dossier</button>
      </div>
      ...
      {open !== null && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50" onClick={(e) => { if (e.target === e.currentTarget) setOpen(null); }}>
          <div className="card p-6 w-[520px] max-w-[92vw]">
            ...
            <input type="number" className="inp w-full px-3 py-2.5 mb-4" value={form.price} onChange={(e) => setForm({ ...form, price: parseInt(e.target.value) || 0 })} />
            <button onClick={async () => { await addMedia(current, open, { name: form.name || "Nouveau média", type: form.type, lvl: form.lvl, price: form.price, link: form.link }); setOpen(null); }} className="btn w-full py-2.5 font-bold">Ajouter au coffre</button>
          </div>
        </div>
      )}
    </div>
